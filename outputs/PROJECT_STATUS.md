...
- [ ] הרצה מלאה של שכבת extraction מול API מתוך סביבת פיתוח עם גישת אינטרנט
- [x] בניית transformations וטבלת country-year עם בדיקות מקומיות
- [ ] הרצה מלאה של ה־ETL מול API ובדיקת קבצי ה־raw וה־processed בפועל
...
ה־transformation נבנה ונבדק מקומית: הוא קורא raw pages, מסנן אגרגטים, בונה
טבלת country-year רחבה ושומר data-quality report. הבדיקה כוללת במפורש את
`USA` לצד `World` ומוודאת שהאגרגט אינו נכנס לפלט.

הצעד הנכון הבא הוא הרצה מלאה של ה־ETL מול ה־API ובדיקה של קבצי ה־raw וה־processed
שנוצרו בפועל. רק לאחריה נוסיף שכבת metrics, ואז דשבורד Streamlit.
